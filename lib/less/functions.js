...
tree.functions = {
    ...
    lightness: function (color, amount) {
        var c = color.toHSL();
        if (amount) {
            return this.hsl(c.h, c.s, amount);
        } else {
            return new(tree.Dimension)(Math.round(c.l * 100), '%');
        }
    },
    ...
    lighten_hsv: function (color, amount) {
        var hsv = color.toHSV(),
            persent = amount.value / 100;
    ...
    darken_hsv: function (color, amount) {
        var hsv = color.toHSV(),
            persent = amount.value / 100;
